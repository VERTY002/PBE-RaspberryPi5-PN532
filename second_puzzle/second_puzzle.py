...
class GraphicPuzzle(Gtk.Window):
    ...
    def uid_thread(self):
        ...
        thread = threading.Thread(target=self.read_uid, daemon=True)
        thread.start()
    ...
    def read_uid(self):
        ...
        if uid:  # if UID is not null
            GObject.idle_add(self.update_label, f"UID: {uid}", "red")
        else:  # if UID is null
            GObject.idle_add(self.update_label, "NO NFC card detected in 10 seconds, the time has expired", "blue")
